...
class FullyqualifieddomainValidationException implements Exception {
  final List<String> errors;

  FullyqualifieddomainValidationException(this.errors);

  @override
  String toString() => 'FullyqualifieddomainValidationException: ${errors.join("; ")}';
}
...
/// Fully Qualified Domain API client.
///
/// For more information, visit: https://apiverve.com/marketplace/fullyqualifieddomain?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [ip] (required) - The IP address to get the fully qualified domain name of [format: ip]
class FullyqualifieddomainClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'ip': ValidationRule(type: 'string', required: true, format: 'ip'),
  };

  FullyqualifieddomainClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/fullyqualifieddomain',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [FullyqualifieddomainValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw FullyqualifieddomainValidationException(errors);
    }
  }

  /// Execute a request to the Fully Qualified Domain API.
  ///
  /// Parameters are validated before sending the request.
  Future<FullyqualifieddomainResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw FullyqualifieddomainException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return FullyqualifieddomainResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw FullyqualifieddomainException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw FullyqualifieddomainException('Resource not found');
      } else {
        throw FullyqualifieddomainException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is FullyqualifieddomainException) rethrow;
      throw FullyqualifieddomainException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Fully Qualified Domain API client.
class FullyqualifieddomainException implements Exception {
  final String message;

  FullyqualifieddomainException(this.message);

  @override
  String toString() => 'FullyqualifieddomainException: $message';
}
